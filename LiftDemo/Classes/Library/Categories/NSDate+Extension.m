...
NSDateComponents * componentsForTimeStap (NSTimeInterval time) {
    NSDateComponents * cmp = [NSDateComponents new];
    NSInteger panding = time;
    cmp.day = 0;
    cmp.hour = 0;
    cmp.minute = 0;
    if (panding < 60) {
        cmp.second = panding;
    } else if (panding < 3600) {
        cmp.minute = panding / 60;
        cmp.second = panding % 60;
    } else if (panding < 86400) {
        NSInteger hour = panding / 3600;
        NSInteger left = panding - hour * 3600;
        cmp.hour = hour;
        cmp.minute = left / 60;
        cmp.second = left % 60;
    } else {
        NSInteger day = panding / 86400;
        panding = panding - day * 86400;
        NSInteger hour = panding / 3600;
        NSInteger left = panding - hour * 3600;
        cmp.day = day;
        cmp.hour = hour;
