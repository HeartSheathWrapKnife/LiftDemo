...
- (void)setData:(JYScrollItemModel *)data
{
    _data = data;
    if (data.isUrl) {
        [self.imageView sd_setImageWithURL:[NSURL URLWithString:self.data.imageName] placeholderImage:data.placeHolder.length ? [UIImage imageNamed:data.placeHolder] : nil];
    } else {
        if (data.image) {
