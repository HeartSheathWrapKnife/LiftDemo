...
// 选中的菜单ID
NSString * const YZSelectMenuIDNote = @"YZSelectMenuIDNote";
...
@property (nonatomic, assign) BOOL didScrollToTop;
@end
...
- (UIView *)coverView
{
    if (_coverView == nil) {
        ...
        if (self.isScrollTop) {//因为源码是取superview 我的是加在tableviewheader上面的  这样取有问题 故直接设置高度
            
            _coverView = [[YZCover alloc] initWithFrame:CGRectMake(coverX, coverY, coverW, ScreenHeight - self.height)];
        } else {
...
- (void)setup
{
    ...
    _showUpArrowOnSelectBtn = NO;
    
    // 监听更新菜单标题通知
    __weak typeof (self) wself = self;

    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(listToTop) name:@"listToTop" object:nil];
    _observer = [[NSNotificationCenter defaultCenter] addObserverForName:YZUpdateMenuTitleNote object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        if (![self.controllers containsObject:note.object]) return ;
        // 获取列
...
//        
        ...
        // 发起回调
        !self.didSetBtnTitleBlock ? : self.didSetBtnTitleBlock([self.menuButtons copy]);
    }];
}
...
- (void)layoutSubviews
{
    ...
    CGFloat insets = self.contentInsets.left + self.contentInsets.right;
    
    CGFloat btnX = 0;
    CGFloat btnY = 0;
    CGFloat btnW = (self.bounds.size.width - insets) / count;
    CGFloat btnH = self.bounds.size.height;

    for (NSInteger i = 0; i < count; i++) {
        // 设置按钮位置
        UIButton *btn = self.menuButtons[i];
        btnX = self.contentInsets.left + i * btnW;
...
- (void)dismiss
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        view.hidden = YES;
        view.backgroundColor = _coverColor;
    }];
    _open = NO;
}
...
- (void)btnClick:(UIButton *)button
{
    ...
    if (self.scrollTop) {
        BLOCK_SAFE_RUN(self.topBlock,button.isSelected);
        [RACObserve(self, self.didScrollToTop)
         subscribeNext:^(NSString* x){
             if (button.selected == YES) { // 当按钮选中，弹出蒙版
                 ...
                 if (self.isShowUpArrowOnSelectBtn) {
                     self.upArrow.hidden = NO;
                     self.upArrow.centerX = button.centerX;
                     self.upArrow.maxY = button.maxY;
                 }
...
- (UIImageView *)upArrow {
    if (!_upArrow) {
        UIImageView * imageView = [UIImageView imageViewWithImage:[UIImage imageNamed:@"icon_a_raised_n"] frame:Rect(0, 0, 11, 6)];
        imageView.hidden = YES;
        [self addSubview:imageView];
        [self bringSubviewToFront:imageView];
        self.upArrow = imageView;
    }
    return _upArrow;
}
#pragma mark - 界面销毁
