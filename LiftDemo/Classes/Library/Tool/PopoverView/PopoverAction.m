...
@property (nonatomic, assign, readwrite) NSInteger index; ///< 下标
...
+ (instancetype)actionWithImage:(UIImage *)image index:(NSInteger)index title:(NSString *)title handler:(void (^)(PopoverAction *action))handler {
    PopoverAction *action = [[self alloc] init];
    action.image = image;
    action.title = title ? : @"";
    action.index = index;
